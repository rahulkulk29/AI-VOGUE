...
class TShirtCustomizer {
    ...
    async init() {
        try {
            ...
            if (this.mockData.products.length > 0) {
                this.selectProduct(this.mockData.products[0], false);
            }
    ...
    async loadMockData() {
        try {
            // Use config path if available, otherwise use relative path
            const mockDataPath = window.TSHIRT_APP_CONFIG?.mockDataPath || 'assets/mock-data.json';
            ...
            this.showToast('Could not load product data. Please refresh the page.', 'error');
        }
    }
    ...
    initCanvas() {
        ...
        // Ensure canvas element offsets are accurate for mouse positioning
        this.canvas.calcOffset();

        // Recalculate canvas offset on window scroll or resize
        const updateOffset = () => {
            if (this.canvas) {
                this.canvas.calcOffset();
            }
        ...
        // Helper to reliably reset any active drag / transform state
        const resetDragState = () => {
            if (this.canvas) {
                if (this.canvas._currentTransform) {
                    try {
                        this.canvas._finalizeCurrentTransform();
                    } catch (err) {
                        // ignore error if transform was already finalized
                    }
        ...
        // Global safety listeners: Catch mouseup, pointerup, pointercancel, mouseleave, blur
        // anywhere on window/document to guarantee graphic stops following cursor when released.
        const globalReleaseHandler = () => {
            if (this.canvas && this.canvas._currentTransform) {
                resetDragState();
            }
        };

        window.addEventListener('mouseup', globalReleaseHandler, true);
        window.addEventListener('pointerup', globalReleaseHandler, true);
        window.addEventListener('pointercancel', globalReleaseHandler, true);
        window.addEventListener('mouseleave', globalReleaseHandler, true);
        window.addEventListener('blur', globalReleaseHandler, true);
     ...
     * Select a product
     */
    selectProduct(product, scrollTo = true) {
    ...
    setupMenuToggle() {
        ...
        if (!menuToggle || !navDrawer || !overlay || !closeBtn) {
