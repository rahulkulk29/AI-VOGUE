...
function initializePrismAI() {
    ...
    if (window.location.pathname.includes('prism-ai-3.0.html')) {
        // Prism AI will auto-initialize via its own script
        console.log('Prism AI page detected - initialization handled by prismAI.js');
    }
}

// Global Authentication Integration
function initializeGlobalAuth() {
    // Resolve path relative to main.js
    let authPath = './auth/global-auth.js';
    const mainScript = document.querySelector('script[src*="main.js"]');

    if (mainScript) {
        const src = mainScript.src;
        authPath = src.substring(0, src.lastIndexOf('/')) + '/auth/global-auth.js';
    }

    // Load global auth manager dynamically
    import(authPath)
        .then(module => {
            console.log('✅ Global authentication initialized');
            // The global auth manager will automatically update all profile icons
        })
        .catch(error => {
            console.warn('⚠️ Global auth not available:', error);
            // Fallback: basic profile button functionality
            setupBasicProfileButtons();
        });
}

// Fallback profile button setup (if global auth fails to load)
function setupBasicProfileButtons() {
    const profileButtons = document.querySelectorAll(
        '.account-link, .gucci-icon[aria-label="Account"], [aria-label="Account"]'
    );
